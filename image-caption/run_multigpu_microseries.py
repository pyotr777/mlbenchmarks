...
def GPUisFree(i):
    command = "nvidia-smi pmon -c 3 -d 1 -i {} -s u".format(i)
...
for run in range(runs):
    batchsizes = [2, 4, 8, 12, 16, 20, 24]
    learnrates=[0.0001, 0.0005, 0.001, 0.002, 0.003, 0.004]
    for batch in batchsizes:
        for lr in learnrates:
            logfile=os.path.join(logdir,"imagecaption_b{}_l{:.5f}_smp{:d}_{:02d}.log".format(batch,lr,samples,run))
            if os.path.isfile(logfile):
                print("file",logfile,"exists.")
                continue
            comm = "python train_gen.py --logger_comment {sm}smps_BS{bs}_LR{lr}_WD0.001 --early_stopping --batch_size {bs} -l {lr} --weight_decay 0.001 --model resnet50 --use_samples {sm} --max_epoch {ep} --first_save 4 --save_step 4".format(bs=batch, lr=lr, sm=samples, ep=epochs)
            print(comm)
